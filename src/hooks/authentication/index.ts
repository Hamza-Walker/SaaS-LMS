...
export const useAuthSignUp = () => {
    ...
    const onGenerateCode = async (email: string, password: string) => {
        ...
        try {
            ...
            // Catch specific error and display it
            if (error.errors) {
    ...
    const onInitiateUserRegistration = handleSubmit(async (values) => {
        ...
        try {
            ...
            if (completeSignUp.status !== "complete") {
                return toast("Error", {
                    description:
                        "Oops! something went wrong, completeSignUp.status incomplete",
                })
            }
            ...
            setCreating(false) // Ensure the loader stops if there's an error
        }
    })
